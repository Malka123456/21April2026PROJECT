...
func (s *UserService) SignUp(input dto_.SignUp) (string, error) {
	...
	return s.Auth.GenerateToken(user.ID) //generate token after creating user
}
...
func (s UserService) GetVerificationCode(e models.User) error {
...
func (s UserService) CreateProfile(id uint, input dto_.ProfileInput) error {
...
func (s UserService) BecomeSeller(id uint, input dto_.SellerInput) (string, error) {
	...
	// update user
	seller, err := s.Repo.UpdateUser(id, models.User{
	...
	// generating token
	token, err := s.Auth.GenerateToken(seller.ID)  // later

	// create bank account information

	err = s.Repo.CreateBankAccount(models.BankAccount{
...
func (s UserService) FindCart(id uint) ([]models.Cart, float64, error) {
...
func (s UserService) GetOrderById(id uint, uId uint) (models.Order, error) {
